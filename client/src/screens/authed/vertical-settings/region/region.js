    ...
    function handleSave() {
        if (objectId !== "") {
            ...
            for (var i = 0; i < provinceId.length; i++) {
                ...
                axios.post(ApiService.getBaseUrl() + '/provinces/getSpecificProvinceById', provinceObj)
                    .then((response) => {
            ...
            for (var j = 0; j < previousProvinceId.length; j++) {
                if (!provinceId.includes(previousProvinceId[j])) {
                    ...
                    axios.post(ApiService.getBaseUrl() + '/provinces/getSpecificProvinceById', objec)
                        .then((response) => {
                            var data = response.data.content[0];
                            var regionsIds = [...data.regionId];
                            var arr = []
            ...
            if (name !== "" && abbreviation !== "") {
                ...
                axios.post(ApiService.getBaseUrl() + '/regions/postRegion', obj)
                    .then((res) => {
                        if (res.data) {
                            ...
                            <Grid container spacing={3}>
                                ...
                                <Grid item xs={12}>
                                    <ButtonGroup variant="contained" color="secondary" size={'large'}>
                                        {addForm &&
                                            <Button onClick={addRegion}><AddIcon style={{ marginRight: "2px" }} /> Add</Button>
                                        }
                                        {!selectedData &&
                                        ...
                                        {isEditingSelectedData &&
                                            <Button onClick={deleteRegion}><DeleteOutlineIcon style={{ marginRight: "5px" }} /> Delete</Button>
                                        }
                                    </ButtonGroup>
                                </Grid>
